...
export const sendEmail = async (payload) => {
  ...
  // Send email
  const response = _.first(await sgMail.send(message));
...
export const checkSendgridHealth = async () => {
  const { data } = await axios("https://status.sendgrid.com/api/v2/summary.json");
  const { status } = _.find(_.get(data, "components", []), { name: "Mail Sending" });
