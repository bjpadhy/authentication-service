...
export const signInUser = async (payloadData) => {
  ...
  const { assert_password, ...user } = await verifyUserPassword(email, password);
  ...
  // Throw 401 for incorrect password
  throw new UnauthorizedError("Incorrect password", { email });
};
